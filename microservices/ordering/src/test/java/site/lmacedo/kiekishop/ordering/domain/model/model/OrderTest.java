...
import site.lmacedo.kiekishop.ordering.domain.model.exception.OrderInvalidShippingDeliveryDateException;
...
class OrderTest {
    ...
    void shouldAddItem() {
        ...
        order.addItem(mouse, new Quantity(1));
    ...
    void shouldCalculateTotals() {
        Order order = Order.draft(new CustomerId());
        order.addItem(ProductTestDataBuilder.aProductAltMouse().build(), new Quantity(2));
    ...
    void givenDraftOrder_whenChangeBilling_shouldAllowChange() {
        ...
        Assertions.assertThat(order.billing()).isEqualTo(OrderTestDataBuilder.aBilling());
    }
    ...
    @Test
    void givenDraftOrderAndDeliveryDateInThePast_whenChangeShipping_shouldAllowChange() {
    ...
    void givenDraftOrder_whenChangeItem_shouldRecalculate() {
        ...
        Assertions.assertWith(order,
                o -> Assertions.assertThat(o.totalAmount()).isEqualTo(new Money("15000.00")),
                o -> Assertions.assertThat(o.totalItems()).isEqualTo(new Quantity(5))
        );
    }

    @Test
    void givenOutOfStockProduct_whenTryToAddItemToAnOrder_shouldGenerateException() {
        Order order = Order.draft(new CustomerId());
